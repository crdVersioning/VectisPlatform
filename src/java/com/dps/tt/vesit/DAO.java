...
{    
    ...
    public static TrackingData readTrackingData(Date fromDate, Date toDate, String region)
    {
        ...
        try
        {
            td.deliveries_dbr = dbi.read("replica.view_Shipments_VESIT").top(1000)
        ...
        for(Record delivery : td.deliveries_dbr)
        {
            ...
            if(deliveryDate!=null && !NULLDATE.equals(SDF.format(deliveryDate))) status = "CONSEGNATO IL "+SDF.format(deliveryDate);
            else
            {
